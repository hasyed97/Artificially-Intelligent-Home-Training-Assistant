...
labl = ['Abs', 'Background', 'Bench Press', 'Down', 'Push ups',  'SidePlank', 'Squats','Up']
folder ="/home/ahmedhasan/Desktop/FYP/NewDataSet/Real Data M7/testing GYM-Data/"
count = 1
lablcount1 = 6
framenumpy =[]
Actual = []
...
for lablecount in range(8):
    for filename in os.listdir(folder+labl[lablecount]):
        frame = cv2.imread(os.path.join(folder+labl[lablecount],filename))
        if frame is not None:
            ...
            for idx, keyPoint in enumerate(personclass.keyPoints):
                ...
                newDF.at[1, keypointsMapping[idx]] = (float("{:.3f}".format(get_Euclideandistance(posi.x, posi.y,
                                                                                                  personclass.keyPoints[
                                                                                                      0].position.x,
                                                                                                  personclass.keyPoints[
                                                                                                      0].position.y)))) / normalization_scale
                # inputArray.append(distance)
            ...
            Actual.append(labl[lablecount])
            predicted.append(label)
            if labl[lablecount]==abclab:
...
print("Accuracy ")
print (Accuracycount/len(Actual)*100)
